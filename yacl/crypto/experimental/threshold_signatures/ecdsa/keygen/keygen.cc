...
KeygenRound2Out KeygenParty::MakeRound2(
  ...
  if (round2_done_) {
    TECDSA_THROW_LOGIC("MakeRound2 must not be called twice");
  }
  if (!round1_done_) {
    TECDSA_THROW_LOGIC("MakeRound1 must be completed before MakeRound2");
  }
...
KeygenRound3Msg KeygenParty::MakeRound3(
  ...
  local_X_i_ = ECPoint::GeneratorMultiply(local_x_i_);
  round3_done_ = true;
  return KeygenRound3Msg{
      .X_i = local_X_i_,
      .proof = core::proof::BuildSchnorrProof(core::DefaultEcdsaSuite(),
                                              cfg_.session_id, cfg_.self_id,
                                              local_X_i_, local_x_i_),
...
KeygenOutput KeygenParty::Finalize(const PeerMap<KeygenRound3Msg>& peer_round3) {
  ...
  public_data.all_X_i[cfg_.self_id] = local_X_i_;
