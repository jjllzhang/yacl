...
constexpr char kPiLinearGroupProofName[] = "PiLinearGroup";
constexpr char kPiLinearProofName[] = "PiLinear";
...
Scalar BuildPiLinearGroupChallenge(const MtaProofContext& ctx, const BigInt& n,
                                   const BigInt& gamma,
  ...
  AppendCommonMtaTranscriptFields(&transcript, kPiLinearGroupProofName, ctx);
...
Scalar BuildPiLinearChallenge(const MtaProofContext& ctx, const BigInt& n,
                              const BigInt& gamma, const AuxRsaParams& aux,
                              const BigInt& c1, const BigInt& c2,
                              const PiLinearProof& proof) {
  core::transcript::Transcript transcript(ctx.transcript_hash);
  AppendCommonMtaTranscriptFields(&transcript, kPiLinearProofName, ctx);
  const Bytes n_bytes = core::encoding::EncodeMpInt(n);
  const Bytes gamma_bytes = core::encoding::EncodeMpInt(gamma);
  const Bytes n_tilde_bytes = core::encoding::EncodeMpInt(aux.n_tilde);
  const Bytes h1_bytes = core::encoding::EncodeMpInt(aux.h1);
...
PiLinearGroupProof ProvePiLinearGroup(const MtaProofContext& ctx,
  ...
  while (true) {
    ...
    const Scalar e_scalar = BuildPiLinearGroupChallenge(
        ctx, n, gamma, verifier_aux, c1, c2, statement_x, proof);
...
PiLinearProof ProvePiLinear(const MtaProofContext& ctx, const BigInt& n,
  ...
  while (true) {
    ...
    const Scalar e_scalar =
        BuildPiLinearChallenge(ctx, n, gamma, verifier_aux, c1, c2, proof);
    const BigInt e = e_scalar.mp_value();
